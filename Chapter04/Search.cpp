...
// Why this has no head in the book code?
// I believe its only for demonstration and not included in the game
 ...
 *
 * Simple, but searches in all directions. Can guarantee a minimal path
 */
...
void testBFS()
{
    ...
    // this returns a reverse path from the goal to the start
    // two ways to handle it
    // 1. reverse the path with a stack
    // 2. more intelligent, reverse the search => from goal to start instead
}

/*
 * Compute heuristics
 *
 * this can be either Manhatten (only from the side of the square) or Euclidean (diagonal allowed)
 * it also can vary from node to node,
 * in this case the additional information may be required and stored in eac node
 * (such as the position of the node in the world)
 */
 ...
 *
 * GBFS cannot guarantee a minimal path.
 * Usefull to understand, because it is not far away from A* Search
 */
    ...
    do {
        ...
        for (const WeightedEdge* edge : current->mEdges) {
            ...
            if (!data.mInClosedSet) {
                // Set the adjacent node's parent
                data.mParentEdge = edge;
                if (!data.mInOpenSet) {
                    // Compute the heuristic for thi node, and add to open set
        ...
        if (openSet.empty())
        {
            break; // Break out of outer loop (quit the algorithm)
        }
        ...
        auto iter = std::min_element(
            openSet.begin(),
            openSet.end(),
            [&outMap](const WeightedGraphNode* a, const WeightedGraphNode* b) { return outMap[a].mHeuristic < outMap[b].mHeuristic; });
...
/*
 * GameTrees with Minimax algorithm ( e.g. for tic tac toe)
 */
...
/*
 * GameTrees with Alpha Beta Pruning algorithm ( e.g. for tic tac toe)
 * Optimized algorithm which does not search the whole game tree
 */
...
const GTNode* AlphaBetaDecide(const GTNode* root) {
    ...
    // Alpha starts at negative infinity, beta a positive infinity
    float maxValue = -std::numeric_limits<float>::infinity();

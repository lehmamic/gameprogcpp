...
//  Game-mac
//
//  Created by Michael Lehmann on 20.01.21.
...
void AABB::Rotate(const Quaternion& q)
{
    ...
    // Permutations with 1 min and 2 max
...
bool Intersect(const AABB& a, const AABB& b)
{
    ...
    // if non of these are true, they must intersect
    return !no;
}
...
bool Intersect(const LineSegment& l, const Sphere& s, float& outT)
{
    ...
    float b = 2.0f * Vector3::Dot(Y, Y);
    float c = Vector3::Dot(X, X);
    ...
    else
    {
        ...
        // check whether either t is within bound of segment
...
bool Intersect(const LineSegment& l, const Plane& p, float& outT)
{
    ...
    {
        // The only way they intersect if start/ende are
        // ppints on teh plane (P dot N) == d
